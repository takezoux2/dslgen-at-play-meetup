...
class ServiceGen extends Generator[Controller] {
  ...
  def code( c : Controller) = {
    ...
    s"""package com.geishatokyo.dslgen.service
       ...
       | *
       | */
       |object ${name}Service extends I${name}Service {
       |
       |${methods}
       |
       |}
   ...
   def generateInterface(c: Controller): (String, String) = {
     ("I" + c.name + "Service.scala") -> codeInterface(c)
   }

   def codeInterface( c : Controller) = {
